...
def count_words(count_dict, text):
    ...
    for sen in text:
        for _word in sen.split():
            if _word not in count_dict:
                count_dict[_word] = 1
            else:
                count_dict[_word] += 1


def write(filename, text):  # Writes data to a file
    with open(filename, 'wb') as fp:
        pickle.dump(text, fp)


def read(filename):  # Reads data from a file
...
for e in parse(file):
    ...
    if s_start != -1 and l_start != -1:
        ...
        sentences.append(sentence)  # Cleaned Review
        labels.append(label)  # Cleaned Summary
...
print('Word embeddings in numberbatch:', len(embeddings_index))
...
# Create matrix with the word embeddings
word_embedding_matrix = np.zeros((nb_words, embedding_dim), dtype=np.float32)
for word, i in vocab_to_int.items():
    if word in embeddings_index:
        word_embedding_matrix[i] = embeddings_index[word]
    else:
        # Create random embedding if word does not exist
...
print("Total number of words in Dataset:", word_count)
print("Total number of UNKs in Dataset:", unk_count)
